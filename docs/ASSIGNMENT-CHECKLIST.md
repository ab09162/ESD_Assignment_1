...
Status: **Verified Docker** means an actual container/API/Prometheus/Elasticsearch check ran on this machine; **Documented** means an explanation was reviewed. Browser rendering/screenshots remain manual. See `VERIFICATION.md` and the linked raw evidence for exact scope and limits.
...
| 7 Throughput/load dashboard | k6 VUs and client tails, server tails/rates, in-flight, resources, DB | `campus-load.json` | Remote-write baseline | 14 load panels/queries verified with real remote write; screenshot manual |
...
| 11 Filebeat→Elasticsearch→Kibana | Shared volume, NDJSON, explicit typed mapping, data view setup/KQL | Filebeat/Elastic/Kibana config, scripts | Filebeat `test config`; `verify-stack.py`, Discover | Verified Filebeat→Elasticsearch; Kibana view/mappings verified, screenshot manual |
...
| 13 Metric end-to-end | Exact observation function, value, exposition, scrape/storage/query/panel | Report Part D | Follow one actual sample | Verified actual sample/query; screenshot manual |
| 14 Log end-to-end | ID→JSONL→parsed fields→index→KQL; verifier emits real stored hit | Report Part C/D, verifier | Run smoke/verifier + Discover | Verified actual stored hit and request ID; screenshot manual |
| 15 Architecture/protocols/storage/failure effects | Mermaid diagrams and eight-component failure matrix | README, report Part D | Read; controlled stop/start | Documented; DB and observability outage/recovery verified |
...
| 17 Increasing users experiment | 10/25/50/100/200 VUs with plateaus; comparison instructions | `load.js`, report E1 | Load dashboard/time ranges | Verified real staged run; 200-user degradation honestly recorded |
...
| 24 Observability failure independence | App startup depends only on PostgreSQL; local queued logs and pull metrics | App + Compose; report matrix | Stop Grafana/Filebeat, smoke | Verified booking with Grafana/Filebeat stopped and subsequent log replay |
...
| 26 Report Parts A–E | Detailed metrics table, logs, design, two experiments, honest placeholders | `report.md` | Read alongside evidence | Updated with real Docker observations; only manual screenshot markers remain |
...
| 28 Screenshot checklist | Actions and evidence for every required chart/search/experiment | `docs/DEMO-CHECKLIST.md` | Follow checklist | Exact historical browser ranges/panels/KQL supplied; screenshots manual |
...
| 45 Autonomous completion and honest errors | Implementation/config/tests/docs/experiments carried through; limitations disclosed | Verification record | Inspect results/pending tasks | Docker work completed; remaining browser evidence explicit |

All requested Docker runtime experiments are now recorded with real evidence. Remaining submission work is manual browser rendering/screenshot capture and publishing the local commit to the user-selected GitHub repository. Seven-day deletion and production capacity are not claimed.
